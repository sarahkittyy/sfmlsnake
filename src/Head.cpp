...
Head::Head(Map *map, QuestionManager *qum) {
	// Set the map.
	mMap = map;
	qm   = qum;
...
void Head::reset() {
	...
	mPos			= {10, 10};
	mFood			= 0;
	mUpdateInterval = sf::milliseconds(90);
...
void Head::update() {
	if (!qm->doneAskingQuestion()) {
		updateFlag = true;
		return;
	} else {
		if (updateFlag) {
			if (qm->lastQuestionCorrect()) {
				mFood += 3;
			} else {
				mFood--;
				if (mFood < 0) {
				...
				mUpdateInterval -= sf::milliseconds(12);
				if (mUpdateInterval < sf::milliseconds(10)) {
					mUpdateInterval = sf::milliseconds(10);
				}
			}
		}
...
bool Head::testMovement() {
	...
	if (mPos.x >= Map::MAP_SIZE.x || mPos.x < 0 || mPos.y < 0 ||
